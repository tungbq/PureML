...
var import_airtable = __toESM(require("airtable")), base = new import_airtable.default({
  endpointUrl: "https://api.airtable.com",
  apiKey: process.env.NEXT_PUBLIC_AIRTABLE_API_KEY
}).base("appAR7Cxhflh7YVe9"), auth_server_default = base, backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL, makeUrl = (path) => `${backendUrl}${path}`;
...
var import_jsx_runtime31 = require("react/jsx-runtime"), action4 = async ({ request }) => {
  let comment = (await request.formData()).get("comment"), accessToken = (await getSession(request.headers.get("Cookie"))).get("accessToken"), email = (await fetchUserSettings(accessToken))[0].email, submit = auth_server_default("tbl7qXTTBN3Ln6KyE").create(
    [
      {
        fields: {
          Email: email,
